![Demo](demo.gif)

# How to play

## Generating map

Go to "Map Creation" tab
Use the sliders to set up your game
Click `Generate`
Select which tiles should have walls
Click `Save`

## Playing

After creating the map, go to the "Active Game"
Click `Load`
Play the game
After the game ends, click `Play again` to reset the game

## Missing features:

- ~~Game ends when 1 player gets blocked in (also when there are more than 2 players -> shouldnt be the case)~~
- ~~Display who won or if its a tie~~
- Everything is done

## Missing Additional features (not needed):

- Change hover color of buttons (50% alpha so player color is still visible)
- Change line style between tiles (or remove lines and add black borders to buttons)
